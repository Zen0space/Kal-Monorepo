 ...
 */
export async function searchKaloriApi(
  query: string
): Promise<KaloriFoodResult[]> {
  ...
  try {
    ...
    const halalRes = await fetch(
      `${API_BASE}/api/v1/halal/search?q=${encodeURIComponent(searchQuery)}`,
      {
    ...
    if (halalRes.ok) {
      const halalData = (await halalRes.json()) as {
        count?: number;
        data?: KaloriFoodResult[];
      };
      if (halalData.data && halalData.data.length > 0) {
        console.log(
          "[Tool: api_checker] Found",
          halalData.count || 0,
          "halal results"
        );
    ...
    console.log(
      "[Tool: api_checker] No halal results, searching natural foods..."
    );
    const foodsRes = await fetch(
      `${API_BASE}/api/v1/foods/search?q=${encodeURIComponent(searchQuery)}`,
      {
        headers: { "X-API-Key": INTERNAL_API_KEY },
      }
    );
    ...
    const foodsData = (await foodsRes.json()) as {
      count?: number;
      data?: KaloriFoodResult[];
    };
    console.log(
...
export async function searchMultipleFoods(
  foodNames: string[]
): Promise<Map<string, KaloriFoodResult | null>> {
  ...
  const searchSingleQuery = async (
    query: string
  ): Promise<KaloriFoodResult | null> => {
    try {
      // Try halal first
      const halalRes = await fetch(
        `${API_BASE}/api/v1/halal/search?q=${encodeURIComponent(query)}`,
        {
          headers: { "X-API-Key": INTERNAL_API_KEY },
        }
      );
      ...
      // Try natural foods
      const foodsRes = await fetch(
        `${API_BASE}/api/v1/foods/search?q=${encodeURIComponent(query)}`,
        {
          headers: { "X-API-Key": INTERNAL_API_KEY },
        }
      );
  ...
  for (const batch of batches) {
    const batchPromises = batch.map(async (foodName) => {
      ...
      if (result) {
        console.log(
          `[Tool: api_checker] Found "${foodName}" with original query`
        );
      ...
      if (normalizedQuery !== originalQuery) {
        console.log(
          `[Tool: api_checker] AI normalized: "${originalQuery}" -> "${normalizedQuery}"`
        );
        result = await searchSingleQuery(normalizedQuery);
        if (result) {
          console.log(
            `[Tool: api_checker] Found "${foodName}" with normalized query`
          );
          return { name: foodName, result };
        }
      }
...
export async function searchRecipeIngredients(
  ...
  Array<KaloriFoodResult & { originalName: string; quantity: string }>
> {
  const results: Array<
    KaloriFoodResult & { originalName: string; quantity: string }
  > = [];
  ...
  for (const ingredient of ingredients) {
    ...
    if (dbResult) {
      ...
      console.log(
        `[Tool: api_checker] Estimating nutrition for "${ingredient.name}"`
      );
      try {
        ...
        console.log(
          `[Tool: api_checker] Failed to estimate "${ingredient.name}":`,
          error
        );
...
export function isRecipeNutritionQuery(message: string): boolean {
  ...
  const hasNutritionKeyword = nutritionKeywords.some((k) =>
