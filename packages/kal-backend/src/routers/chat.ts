    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (isRecipeNutritionQuery(content)) {
        ...
        if (cachedRecipe) {
          ...
          const ingredients = cachedRecipe.ingredients.map(
            (i: RecipeIngredient) => ({
              name: i.name,
              quantity: i.quantity,
            })
          ...
          console.log(
            "[Tool: recipe] Searching nutrition for",
            ingredients.length,
            "ingredients"
          );
          ...
          console.log(
            "[Tool: recipe] No cached recipe, looking in conversation..."
          );
          ...
          for (const msg of recentAssistantMessages) {
            ...
            if (
              ...
              console.log(
                "[Tool: recipe] Found recipe in conversation, parsing..."
              );
              try {
                ...
                // Search for all ingredients
                const ingredients = parsedRecipe.ingredients.map(
                  (i: RecipeIngredient) => ({
                    name: i.name,
                    quantity: i.quantity,
                  })
                ...
                const nutritionResults =
                  await searchRecipeIngredients(ingredients);
                const totals = calculateTotalNutrition(nutritionResults);
      ...
      // Handle recipe requests - save for later nutrition queries
        ...
        recipeContext =
          "User is asking for a recipe. After providing the recipe, they may ask about its nutrition.";
      }
      // Handle food queries
      else if (intentResult.requires_api_lookup || isFoodQuery(content)) {
        console.log(
          "[Tool: api_checker] Food query detected, searching API..."
        );
      ...
      if (
        ...
        try {
          ...
          if (parsedRecipe.ingredients.length > 0) {
            ...
            console.log(
              "[Tool: recipe] Calculating nutrition for",
          ...
          console.log(
            "[Tool: recipe] Failed to parse/calculate nutrition:",
            error
          );
